                    ...
                    struct vm_rg_struct *ret_rg)
{
    ...
    for (; pgit < pgnum; ++pgit) {
        ...
        if(fpit->type_mem==1)
        {
        	...
        	Enqueue(caller->mram,pgn + pgit,fpit->fpn,caller->mm);
...
int alloc_pages_range(struct pcb_t* caller, int req_pgnum, struct framephy_struct** frm_lst)
{
    ...
    for (pgit = 0; pgit < req_pgnum; pgit++)
    {
        ...
        else if ( MEMPHY_get_freefp(caller->active_mswp, &fpn) == 0)
        {
          ...
          {
            freefp_str = *frm_lst;
            if(freefp_str->type_mem==1) MEMPHY_put_freefp(caller->mram,freefp_str->fpn);
            else MEMPHY_put_freefp(caller->active_mswp,freefp_str->fpn);
